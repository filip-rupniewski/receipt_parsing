...
3. **Install the required Python packages:**
   *(Assuming you have a `requirements.txt` file)*
   ...
   ```bash
   uv add opencv-python pytesseract numpy
   ```
...
```bash
uv run main.py path/to/your_receipts/receipt1.jpg --shop denner
```

**To process an entire directory of Lidl receipts:**

```bash
uv run main.py path/to/your_receipts/ --shop lidl
```

**To run in debug mode:**

```bash
uv run main.py path/to/your_receipts/ --shop denner --debug
```
...
- [ ] correct detection of: Cashew Peanut Mix Hot-Ch    0,2    2,79    13,95    lidl
- [ ] correct weight. Now it is in grams