...
class CoopParser(BaseParser):
    """
    Parses the OCR text from a Coop receipt to extract the date and a list of purchased items.
    Behaves the same as MigrosParser as per requirements.
    """
    ...
    def parse_date(self, text: str) -> Optional[str]:
        ...
        Coop receipts typically have the date at the very bottom.
        ...
        for line in item_lines:
            ...
            if np.isnan(standardized_size):
                if product_name not in size_map:
                    missing_names.add((product_name, "coop"))
                else:
            ...
            item = ReceiptItem(
                ...
                shop="coop",                # ✅ Correct: shop (not store)
