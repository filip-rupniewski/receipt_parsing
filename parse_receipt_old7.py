...
class ReceiptProcessor:
    ...
    def _parse_items(self, text: str, date_str: str) -> List[ReceiptItem]:
        ...
        while i < len(lines):
            ...
            item = ReceiptItem(status_flag, date_str, 0, product_name, standardized_size, price, price_per_one, "denner", discount)
...
def apply_corrections(items: List[ReceiptItem], corrections_map: Dict[str, dict]):
    ...
    for item in items:
        if item.name in corrections_map:
            ...
            # --- MODIFIED SIZE CORRECTION LOGIC ---
            if 'size' in rule:
                corrected_size = rule['size']
                # Case 1: Multiply if an old size exists and is valid
                if not np.isnan(item.size):
                    item.size = item.size * corrected_size
                # Case 2: Replace if no old size exists
                else:
                    item.size = corrected_size
            
            # Recalculate derived fields after ANY correction
...
def apply_naive_corrections(items: List[ReceiptItem]):
    ...
    - For each item 'R' with status '!', it finds all valid items with the same name.
    - It identifies the valid item 'R2' with the closest date to 'R'.
    - Based on what's missing in 'R', it uses data from 'R2' to fill the gaps:
        - Missing Price: Calculates Price from R's size and R2's price_per_one. Status -> '!pP'
        - Missing Size: Calculates Size from R's price and R2's price_per_one. Status -> '!sP'
        - Missing Both: Copies Price, Size, and price_per_one from R2. Status -> '!spP'
    """
    print("\nApplying naive corrections for items with missing data...")
    corrections_count = 0
    
    # 1. Create a map of valid items for quick lookup
    good_items_map = collections.defaultdict(list)
    for item in items:
        # A "good" item has a complete record (no status flag) and a valid date
        if not item.status_flag and item.date != "N/A":
            good_items_map[item.name].append(item)

    # 2. Iterate through all items to find and fix the ones with status '!'
    for item_to_fix in items:
        if item_to_fix.status_flag != "!" or item_to_fix.date == "N/A":
            continue

        # Find candidate items from our map
        candidates = good_items_map.get(item_to_fix.name)
        if not candidates:
            continue # No valid historical data for this product name

        # 3. Find the candidate with the closest date
        try:
            target_date = datetime.strptime(item_to_fix.date, '%d.%m.%Y')
        except ValueError:
            continue # Can't compare if the item's own date is invalid
        ...
        for candidate in candidates:
            try:
                ...
                continue # Skip candidates with invalid dates

        if not closest_match:
            continue

        # 4. Apply the correction based on the missing data
        new_status = ""
        # Scenario 1: Has size, missing price
        if not np.isnan(item_to_fix.size) and np.isnan(item_to_fix.price):
            ...
            new_status = "!pP" # Calculated Price using Price_per_one

        # Scenario 2: Has price, missing size
            ...
            if closest_match.price_per_one and closest_match.price_per_one > 0:
                ...
                new_status = "!sP" # Calculated Size using Price_per_one

        # Scenario 3: Missing both size and price
            ...
            item_to_fix.price_per_one = closest_match.price_per_one
            new_status = "!SPP" # Fixed Size and Price and Price_per_one
...
def display_console_preview(items: List[ReceiptItem]):
    ...
    header = ['status', 'date', 'order number', 'product name', 'size/volume', 'price', 'price per 1', 'shop', 'discount']
    print(f"{header[0]:<7}{header[1]:<12}{header[2]:<14}{header[3]:<35}{header[4]:<13}{header[5]:<10}{header[6]:<13}{header[7]:<10}{header[8]:<10}")
    print("-" * 125)
    for item in items[:20]:
        name_short = (item.name[:32] + '...') if len(item.name) > 35 else item.name
        print(f"{item.status_flag:<7}{item.date:<12}{item.order_number:<14}{name_short:<35}"
...
def save_to_csv(items: List[ReceiptItem]):
    ...
    print(f"\nSaving all data to {filename}..."); header = ['status', 'date', 'order number', 'product name', 'size/volume', 'price', 'price per 1', 'shop', 'discount']
    sorted_items = sorted(items) #we use defined ordering on ReceiptItem
    with open(filename, 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f, delimiter=';'); writer.writerow(header)
        for item in sorted_items:
            row = [item.status_flag, item.date, item.order_number, item.name,
...
def save_correction_template(items: List[ReceiptItem], corrections_map: Dict[str, dict]):
    """
    Saves a template for correcting product names.
    This function combines names from the current scan with names from the
    existing corrections file. If a correction rule already exists for a
    product name, it pre-fills the 'correct product name' and 'correct size'
    fields with the existing values.
    """
    if not items and not corrections_map:
        return  # Nothing to write if both sources are empty
    ...
    # Get unique product names from the current processing batch
    names_from_current_scan = set(item.name for item in items)
    # Get all product names that already have a correction rule
    names_from_existing_corrections = set(corrections_map.keys())
    
    # Combine both sets to get a cumulative list of all unique names ever encountered
    ...
    with open(filename, 'w', newline='', encoding='utf-8') as f:
        ...
        for i, name in enumerate(all_unique_names, start=1):
            # Check if a correction rule already exists for this name
            if name in corrections_map:
                rule = corrections_map[name]
                # Pre-fill with existing correction data
                correct_name = rule.get('name', '')
                correct_size_val = rule.get('size')
                
                # Format the size value for CSV output (handle None and convert float to string)
                correct_size_str = ''
                ...
                # For new items not in the corrections file, write an empty template row
...
def _print_summary_table(title: str, data: List[Tuple[str, int]]):
    ...
    for i, (name, count) in enumerate(data, start=1):
        # Truncate long names to prevent breaking the table format
        name_short = (name[:47] + '...') if len(name) > 50 else name
        print(f"{i:<5} {name_short:<50} {count}")

def display_summary(items: List[ReceiptItem]):
    """Counts and prints product occurrences, sorted in two ways, using a helper."""
    print("\n" + "="*28 + " FINAL SUMMARY " + "="*29) # Adjusted for new width
    
    # 1. Prepare the data (this part is the same)
    ...
    # 2. Delegate printing to the helper function
...
def main():
    ...
    # Load corrections regardless of whether new items were found.
    ...
    if all_items:
        # Apply corrections if there are items to apply them to.
        if corrections_map:
            apply_corrections(all_items, corrections_map)
        #Apply corrections based on similar rows with full data
        apply_naive_corrections(all_items)
        
        print(f"\n--- Finished processing all files. Found a total of {len(all_items)} items. ---")
        
        # Display and save data related to the current run
    ...
    # Save the cumulative correction template. This runs even if all_items is empty.
