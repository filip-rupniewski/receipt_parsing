...
def parse_date_from_text(text):
    """
    Parses only the date from the raw text and returns it in DD.MM.YYYY format.
    ...
    while i < len(lines):
        ...
        if line.endswith('#') and i + 1 < len(lines):
            ...
            if prices_on_next_line:
                ...
                # The total price is ALWAYS the last number on the second line.
                price = float(prices_on_next_line[-1].replace(',', '.'))
                price_per_one = price # Default
...
def main():
    ...
    # --- NEW: Save to 'output_data' subfolder ---
